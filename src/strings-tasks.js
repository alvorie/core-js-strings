...
function isPalindrome(str) {
  ...
  let r = temp.length - 1;
  while (l < r) {
    while (temp[l].match(/[a-z]/i) === null) {
      l += 1;
    }
    while (temp[r].match(/[a-z]/i) === null) {
      r -= 1;
    }
    if (temp[l] !== temp[r]) {
      return false;
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((a) => {
      if (alphabet.includes(a.toLowerCase())) {
        return a.toLowerCase() === a
          ? alphabet[(alphabet.indexOf(a) + 13) % 26]
          : alphabet[
              (alphabet.indexOf(a.toLowerCase()) + 13) % 26
            ].toUpperCase();
      }
      return a;
    })
...
function getCardId(value) {
  ...
  const suitsOrder = ['♣', '♦', '♥', '♠'];
  return (
    typeOrder.indexOf(value.replace(value[value.length - 1], '')) +
    suitsOrder.indexOf(value[value.length - 1]) * 13
  );
}
